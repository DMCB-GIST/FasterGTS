...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 30 09:57:02 2021
...
def save_model(model, generation_index, dir_name, target_cell):
    ...
    torch.save(model, out_dir+'/model_'+target_cell)

def load_saved_model(generation_index,target_cell):

    model = torch.load('./RESULTS/'+str(generation_index)+'/model_'+target_cell)
    model = model.eval()
    return model 
...
def train_and_save_model(smiles_ls, pro_val, generation_index,target_cell,device='cpu'): 
    # print('ACQ ...')
    dataset_x = obtain_discr_encoding(smiles_ls, num_processors=1) # multiprocessing.cpu_count()
    # dataset_y = np.array(pro_val)
    # print('DATA acquired')
    
    avg_val = np.average(pro_val)
    ...
    discriminator = do_x_training_steps(data_x=dataset_x, data_y=dataset_y, net=discriminator, 
                                        optimizer=d_optimizer, loss_func=d_loss_func, steps=4000, 
                                        graph_x_counter=1, device=device)
    
    # Save discriminator after training 
    save_model(discriminator, generation_index=generation_index, dir_name='RESULTS',target_cell=target_cell)
    

def obtain_new_pred(smiles_ls, generation_index,target_cell,device='cpu'): 

    device = device
    predictions = []
    model = load_saved_model(generation_index=generation_index,target_cell=target_cell) 
...
# smiles_ls = ['C', 'CC', 'CCCC', 'CCCCC', 'CCCCCCC']
# pro_val        = [1.0, 1.2, 1.4, 1.6, 1.8]

# # train and save the model: 
# train_and_save_model(smiles_ls, pro_val, generation_index=1)


# # Obtain new predictions: 
# new_predictions = obtain_new_pred(smiles_ls, generation_index=1)

# predictions = []
# model = load_saved_model(generation_index=1) 
