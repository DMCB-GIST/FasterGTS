...
for index in index_list:
    ...
    # df['property'] = ddf.map_partitions(parr_property, meta='float').compute(scheduler='processes')
    # prop_map       = df.set_index('SMILES').to_dict()['property']
    
    prop_map = {}
    for item in unique_pop: 
        prop_map[item] = PlP(item)
    ...
    for gen_ in range(generations): 
        ...
        if gen_ == 0: 
            ...
            train_and_save_model(train_smiles, pro_val, generation_index=gen_,
                                 target_cell=target_cell,device=device)
        ...
        #with open(results_dir_path + '/fitness_local_search.txt', 'a+') as f: 
        #    f.writelines(['{}\n'.format(x) for x in fitness_sort])
        population_sort = [mut_smi_dict_local_calc[x] for x in idx_sort_lc]
        #with open(results_dir_path + '/population_local_search.txt', 'a+') as f: 
        #    f.writelines(['{}\n'.format(x) for x in population_sort])
        
        # Plot of exploration and interpollation fitnesses
        ...
        #with open(results_dir_path + '/exchange_members.txt', 'a+') as f: 
        ...
        df.to_csv('./RESULTS/C50_'+target_cell+'_'+str(gen_)+'_th_results.csv')
