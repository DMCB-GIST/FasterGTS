"""
This code is written by authors of CanDIS.
...
learning_rate = 6e-5 #6e-4
batch_size = 128
max_len = 100

for index in indices:

    mode = 'WO_MCTS_lr_6e5_stop_10000_const_'+str(constant)
    ...
    img_save_dir = './FasterGTS/CanDIS_Trainable_gpt_imgs/'+mode+'_'+target_cell+'_'
    file_save_dir = './FasterGTS/CanDIS_Trainable_gpt_results/'+mode+'_'+target_cell+'_'
    dict_save_dir = './FasterGTS/CanDIS_Trainable_gpt_dict/'+mode+'_'+target_cell+'_'

    config = TrainerConfig(max_epochs= 100, batch_size=batch_size, learning_rate=learning_rate,
    	                      lr_decay=True, warmup_tokens=0.1*1024*max_len,
                              final_tokens=100*1024*max_len, num_workers=10,
    ...
    state = State_trainable_GPT(generator,trainable_generator, predictor, adj_fp_fun,stoi,itos,
                        ...
                        train_batch=128, max_queue=1024, num_self = 60, mutation_rate = 0.01, num_GA =60,
                        optimizer = optimizer, scaler = scaler)

    losses = []
    total_smis = []
    total_rewards = []
    results_df = pd.DataFrame(columns=['smis','rewards'])
    print(target_cell)
    for i in range(100):

        smis, rewards = state.self_train(save_all=True)
        total_smis += smis
        total_rewards += rewards
        try:
            smis, rewards = state.GA(save_all=True)
            total_smis += smis
            total_rewards += rewards
            loss = state.step()
            print('loss: ',loss)
        ...
        if len(total_smis) > 10000:
            break
    results_df = pd.DataFrame({'smis': total_smis,
                                   'rewards': total_rewards})

    results_df = results_df.drop_duplicates().sort_values(by='rewards',ascending=False)

    file_name = '_'+str(i)+'th_final_output.csv'
    results_df.to_csv('./FasterGTS/WO_MCTS_results/'+target_cell+file_name)

    trainable_generator.save_model('./FasterGTS/CanDIS_gpt_weight/'+mode+'_'+target_cell+'.pt')
