...
def generate_samples(generator,itos,stoi,device,n_samples):
    generator.eval()
    x = torch.tensor([stoi['<']], dtype=torch.long)[None,...].repeat(n_samples, 1).to(device)
    y = get_samples(generator, x, generator.block_size-1, sample=True, top_k=40)
    samples = []
    for gen_mol in y:
...
def train(RL_max, generator,target_cell,optimizer,scaler,device,itos,stoi,
    ...
    for i in range(n_policy):
        samples = list(set(generate_samples(generator,itos,stoi,device,n_to_generate)))
        ...
        test_samples = generate_samples(generator,itos,stoi,device,n_try)
    ...
    prl_weight = '/FasterGTS/pre_trained_weights/PRL_gpt_'+target_cell+'.pt'
...
class Personalized_RL(object):
    def __init__(self, generator, predictor, adj_fp_fun, stoi=None,
